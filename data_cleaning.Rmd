...
Once this is done, the `areafy` function will compute the appropriate size metric for each component as per the PDS proforma (available on request).
...
Finally, we need to relabel the columns so they match the column names used by the
Blockbuster Deterioration model in the `Blockbuster2` package.
...
```{r eval = FALSE}
data_files <- clean_PDS(data_files) %>%
  create_element() %>%
  clean_element() %>%
  format_element() %>%
  areafy() %>%
